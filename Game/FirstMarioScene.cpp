...
void FirstMarioScene::init() {
    ...
    for (const EnemySpawn& spawn : enemySpawns) {
        ...
        if (!spawn.killable) {
            enemyRender->getSprite().setColor(sf::Color(255, 110, 110));
        }
    ...
    for (const sf::Vector2f& pos : coinPositions) {
        ...
        Render* coinRender = new Render(*rm->loadTexture("coin.png"), sf::IntRect({ 0, 0 }, { 64, 64 }), { 32.f, 32.f });
    ...
    Render* flagRender = new Render(*rm->loadTexture("flag.png"), sf::IntRect({ 0, 0 }, { 64, 64 }), { 32.f, 32.f });
    ...
    TextRenderer* scoreRender = new TextRenderer({ 1700.f, 60.f }, *uiFont, sf::String("Score : 0"), 48);
    scoreEntity->addComponent(scoreRender);
    scoreEntity->createComponent<ScoreUpdate>();
